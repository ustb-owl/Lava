...
std::optional<std::uint32_t> Evaluator::visit(ProtoTypeAST *ast) { return {}; }
...
std::optional<std::uint32_t> Evaluator::visit(FuncParamAST *ast) { return {}; }

std::optional<std::uint32_t> Evaluator::visit(StructDefAST *ast) { return {}; }
...
std::optional<std::uint32_t> Evaluator::visit(TypeAliasAST *ast) { return {}; }

std::optional<std::uint32_t> Evaluator::visit(StructElemAST *ast) { return {}; }
...
std::optional<std::uint32_t> Evaluator::visit(IfElseStmt *ast) {
  ...
  if (cond)
    ast->set_cond(MakeAST(*cond, ast->cond()));
  // evaluate blocks
  ast->then()->Eval(*this);
  if (ast->else_then())
...
std::optional<std::uint32_t> Evaluator::visit(BinaryStmt *ast) {
  ...
  if (rhs)
    ast->set_rhs(MakeAST(*rhs, ast->rhs()));
  // do not evaluate binary expressions with assign operators
  if (BinaryStmt::IsOperatorAssign(ast->op()))
    return {};
  // evaluate & update lhs
  auto lhs = ast->lhs()->Eval(*this);
  if (lhs)
    ast->set_lhs(MakeAST(*lhs, ast->lhs()));
  if (!lhs || !rhs)
    return {};
  // calculate result
  const auto &type = ast->ast_type();
  ...
  case Op::Sub:
    return lv - rv;
  case Op::Mul:
    return type->IsUnsigned() ? lv * rv : slv * srv;
  case Op::SDiv:
    return type->IsUnsigned() ? lv / rv : slv / srv;
  case Op::SRem:
    return type->IsUnsigned() ? lv % rv : slv % srv;
...
std::optional<std::uint32_t> Evaluator::visit(UnaryStmt *ast) {
  ...
  case Op::Addr:
    return {};
  case Op::SizeOf:
    return ast->opr()->ast_type()->GetSize();
  default:
    assert(false);
    return {};
  }
...
std::optional<std::uint32_t> Evaluator::visit(IndexAST *ast) {
  ...
  if (val)
    ast->set_index(MakeAST(*val, ast->index()));
  return {};
}
...
std::optional<std::uint32_t> Evaluator::visit(CallStmt *ast) {
  ...
  for (std::size_t i = 0; i < ast->args().size(); ++i) {
    ...
    if (val)
      ast->set_arg(i, MakeAST(*val, ast->args()[i]));
  }
  return {};
}
...
std::optional<std::uint32_t> Evaluator::visit(CharAST *ast) { return ast->c(); }

std::optional<std::uint32_t> Evaluator::visit(StringAST *ast) { return {}; }
...
std::optional<std::uint32_t> Evaluator::visit(PrimTypeAST *ast) { return {}; }

std::optional<std::uint32_t> Evaluator::visit(StructTypeAST *ast) { return {}; }

std::optional<std::uint32_t> Evaluator::visit(EnumTypeAST *ast) { return {}; }

std::optional<std::uint32_t> Evaluator::visit(ConstTypeAST *ast) { return {}; }

std::optional<std::uint32_t> Evaluator::visit(PointerTypeAST *ast) {
  return {};
}

std::optional<std::uint32_t> Evaluator::visit(UserTypeAST *ast) { return {}; }
