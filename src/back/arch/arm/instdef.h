...
#define CLASSOF_INST(CLS)                                                      \
  static inline bool classof(LLInst *inst) {                                   \
    return inst->classId() == ClassId::CLS##Id;                                \
  }                                                                            \
  static inline bool classof(const LLInst *inst) {                             \
    return inst->classId() == ClassId::CLS##Id;                                \
  }
...
public:
  ...
  void SetBlocks(std::vector<LLBlockPtr> blocks) {
...
public:
  explicit LLBasicBlock(std::string name, mid::BlockPtr block,
                        LLFunctionPtr parent)
      : _name(std::move(name)), _block(std::move(block)),
...
public:
  ...
  inline static LLOperandPtr Register(ArmReg reg) {
    auto n = (int)reg;
    DBG_ASSERT(n >= int(ArmReg::r0) && n <= int(ArmReg::pc),
               "reg is out of range (r0 to r15)");
  ...
  void set_not_allowed_to_tmp(bool value) { _allow_to_tmp = value; }
...
public:
  LLBinaryInst(Opcode opcode, LLOperandPtr dst, LLOperandPtr lhs,
               LLOperandPtr rhs)
      : LLInst(opcode, ClassId::LLBinaryInstId), _dst(std::move(dst)),
        _lhs(std::move(lhs)), _rhs(std::move(rhs)), _psr(ArmPSR::N) {}
  ...
  void          set_dst(const LLOperandPtr &dst) final { _dst = dst; }
...
public:
  LLMove(LLOperandPtr dst, LLOperandPtr src, ArmCond cond = ArmCond::Any)
      : LLInst(Opcode::Move, ClassId::LLMoveId), _dst(std::move(dst)),
        _src(std::move(src)), _shift(ArmShift()), _cond(cond), _is_arg(false) {}
...
public:
  LLBranch(ArmCond arm_cond, LLOperandPtr cond, LLBlockPtr true_block,
           LLBlockPtr false_block)
      : LLInst(Opcode::Branch, ClassId::LLBranchId), _arm_cond(arm_cond),
        _cond(std::move(cond)), _true_block(std::move(true_block)),
        _false_block(std::move(false_block)), _need_out_false(true) {}
...
public:
  LLLoad(LLOperandPtr dst, LLOperandPtr addr, LLOperandPtr offset)
      : LLInst(Opcode::Load, ClassId::LLLoadId), _dst(std::move(dst)),
        _addr(std::move(addr)), _offset(std::move(offset)), _is_arg(false),
        _cond(ArmCond::Any) {}
...
public:
  LLLoadPL(LLOperandPtr dst, LLOperandPtr addr, LLOperandPtr offset)
      : LLInst(Opcode::Load, ClassId::LLLoadPLId), _dst(std::move(dst)),
        _addr(std::move(addr)), _offset(std::move(offset)) {}
...
public:
  LLLoadChangeBase(LLOperandPtr dst, LLOperandPtr addr, LLOperandPtr offset)
      : LLInst(Opcode::Load, ClassId::LLLoadChangeBaseId), _dst(std::move(dst)),
        _addr(std::move(addr)), _offset(std::move(offset)) {}
...
public:
  LLLoadPseudo(LLOperandPtr dst, int number)
      : LLInst(Opcode::Load, ClassId::LLLoadPseudoId), _dst(std::move(dst)),
        _imm(number) {}
...
public:
  LLStore(LLOperandPtr data, LLOperandPtr addr, LLOperandPtr offset)
      : LLInst(Opcode::Store, ClassId::LLStoreId), _data(std::move(data)),
        _addr(std::move(addr)), _offset(std::move(offset)) {}
...
public:
  LLStoreChangeBase(LLOperandPtr data, LLOperandPtr addr, LLOperandPtr offset)
      : LLInst(Opcode::Store, ClassId::LLStoreChangeBaseId),
        _data(std::move(data)), _addr(std::move(addr)),
...
public:
  LLCompare(ArmCond cond, LLOperandPtr lhs, LLOperandPtr rhs)
      : LLInst(Opcode::Compare, ClassId::LLCompareId), _cond(cond),
        _lhs(std::move(lhs)), _rhs(std::move(rhs)), _is_pl(false) {}
...
public:
  LLCall(mid::FuncPtr function)
      : LLInst(Opcode::Call, ClassId::LLCallId), _function(std::move(function)),
        _is_tail_call(false), _changed_mod(false) {}
...
public:
  LLFMA(Opcode opcode, const LLOperandPtr &dst, const LLOperandPtr &lhs,
        const LLOperandPtr &rhs, const LLOperandPtr &acc, ClassId classId)
  ...
  void set_operand(const LLOperandPtr &opr, std::size_t index) override;
  void set_dst(const LLOperandPtr &dst) override { _dst = dst; }
...
public:
  LLMLA(const LLOperandPtr &dst, const LLOperandPtr &lhs,
        const LLOperandPtr &rhs, const LLOperandPtr &acc)
  ...
  void set_dst(const LLOperandPtr &dst) final { LLFMA::set_dst(dst); }
...
public:
  LLMLS(const LLOperandPtr &dst, const LLOperandPtr &lhs,
        const LLOperandPtr &rhs, const LLOperandPtr &acc)
...
public:
  LLGlobal(const LLOperandPtr &dst, mid::GlobalVariable *glob)
      : LLInst(Opcode::Global, ClassId::LLGlobalId), _dst(dst),
        _glob_var(glob) {}
