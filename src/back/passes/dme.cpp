...
void DeadMoveElimination::runOn(const LLFunctionPtr &func) {
  ...
  if (func->is_decl())
    return;
  if (func->function()->GetFunctionName() == "KMP")
    return;
  if (func->function()->GetFunctionName() == "read_str")
    return;
  if (func->function()->GetFunctionName() == "get_next")
    return;
  if (func->function()->GetFunctionName() == "main")
    return;
  bool changed = true;
  ...
  while (changed) {
    ...
    for (const auto &block : func->blocks()) {
      for (auto it = block->insts().begin(); it != block->insts().end(); it++) {
        if (auto move_inst = dyn_cast<LLMove>(*it)) {
          ...
          // movcond rd, rn, ...
          if (!move_inst->IsSimple())
            continue;
          // mov rd, imm_need_split
          if (src->IsImmediate() &&
              !lava::back::LLModule::can_encode_imm(src->imm_num()))
            continue;
          // mov rd, r0~r3
          if (src->IsRealReg() && (src->reg() <= ArmReg::r3))
            continue;
    ...
    for (auto &[k, v] : _mov_dst_map) {
      if (k->IsRealReg() && (k->reg() <= ArmReg::r3))
        continue;
      changed = true;
