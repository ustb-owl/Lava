...
LLOperandPtr Spill::GetTmpReg(std::uint32_t &reg_mask) {
  ...
  } else if (!(reg_mask & (1 << static_cast<int>(ArmReg::fp)))) {
    reg_mask |= 1 << static_cast<int>(ArmReg::fp);
    temp = LLOperand::Register(ArmReg::fp);
  }
  //  else if (!(reg_mask & (1 << static_cast<int>(ArmReg::r3)))) {
  //    reg_mask |= 1 << static_cast<int>(ArmReg::r3);
  //    temp = LLOperand::Register(ArmReg::r3);
  //  }
  DBG_ASSERT(temp != nullptr, "get tmp register(r12/fp) failed");
  return temp;
}
