...
public:
  ...
  // getters
  const front::LoggerPtr &logger() const { return logger_; }
...
public:
  explicit TranslationUnitDecl(ASTPtrList _decls) : decls_(std::move(_decls)) {}
  ...
  void Merge(ASTPtrList &decls) override {
    decls.insert(decls.end(), std::make_move_iterator(decls_.begin()),
...
public:
  ...
  void set_expr(std::size_t i, ASTPtr expr) { exprs_[i] = std::move(expr); }
...
public:
  ...
  ControlAST(Type type, ASTPtr expr) : type_(type), expr_(std::move(expr)) {}
...
public:
  ...
  UnaryStmt(Operator op, ASTPtr opr) : op_(op), opr_(std::move(opr)) {}
...
public:
  ...
  // setters
  void set_arg(std::size_t i, ASTPtr arg) { args_[i] = std::move(arg); }
