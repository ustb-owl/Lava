...
inline constexpr std::string_view EmitKindName(EmitKind emit) {
  switch (emit) {
    ...
    case EmitKind::Ir:  return "ir";
    case EmitKind::Asm: return "asm";
    case EmitKind::Cfg: return "cfg";
  }
  return "asm";
}
...
inline std::optional<EmitKind> ParseEmitKind(std::string_view emit_name) {
  if (emit_name == "ast") return EmitKind::Ast;
  if (emit_name == "ir") return EmitKind::Ir;
  if (emit_name == "asm") return EmitKind::Asm;
  if (emit_name == "cfg") return EmitKind::Cfg;
  return std::nullopt;
}
