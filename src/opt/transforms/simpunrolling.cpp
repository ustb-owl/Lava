...
public:
  bool runOnFunction(const FuncPtr &F) final {
    ...
    PassManager::RunPassOnFunction("InstSimplify", F);
    PassManager::RunPassOnFunction("StrengthReduction", F);
    PassManager::RunPassOnFunction("LocalValueNumbering", F);
  ...
  void UnrollConst(const FuncPtr &F) {
    ...
    for (const auto &loop : deepest) {
      ...
      if (CheckConstLoop(loop, i_init, end_num, induct_var, update_stmt)) {
        ...
        if (i_init < end_num &&
            (bin_update_stmt->opcode() == BinaryOperator::BinaryOps::Add) &&
            (cmp_inst->op() == front::Operator::SLessEq ||
             cmp_inst->op() == front::Operator::SLess)) {
          if (cmp_inst->op() == front::Operator::SLessEq)
            end_num += 1;
          int update_num =
              dyn_cast<ConstantInt>(bin_update_stmt->RHS())->value();
          loop_time = (end_num - i_init) / update_num;

          if (loop_time * loop->blocks()[1]->insts().size() > 200) {
            continue;
          }
          ...
          if (((end_num - i_init) % update_num) == 0) {
            ...
            for (auto i = 0; i < (loop_time - 1); i++) {
              ...
              for (auto &it : _phi_map) {
                DBG_ASSERT(_phi_tmp_map.find(it.first) != _phi_tmp_map.end(),
                           "failed phi-node failed");
  ...
  bool CheckConstLoop(const Loop *loop, int &i, int &end_num,
                      SSAPtr &induction_var, SSAPtr &update_stmt) {
    auto header = loop->header();
    if (loop->blocks().size() != 3 || loop->blocks().back() != header)
      return false;
    auto term_inst = dyn_cast<BranchInst>(header->insts().back());
    DBG_ASSERT(term_inst != nullptr,
               "the last instruction of loop-condition should be branch");
          ...
          if (auto init =
            ...
            if (auto binary_inst = dyn_cast<BinaryOperator>(new_i)) {
              if (binary_inst->LHS() == phi_node &&
                ...
                // check loop body
                // return false if loop body has call instructions or local
                // array
                int inst_cnt = 0;
                for (const auto &inst : loop->blocks()[1]->insts()) {
                  if (IsSSA<CallInst>(inst) || IsSSA<AllocaInst>(inst) ||
                      ++inst_cnt >= 16)
                    return false;
                }
  ...
  void UnrollLeftConst(const FuncPtr &F) {
    ...
    for (const auto &loop : deepest) {
      ...
      if (CheckLeftConstLoop(loop, i_init, end_var, induct_var, update_stmt)) {
        ...
        if (while_end->predecessor_count() > 1)
          continue;
        ...
        if ((bin_update_stmt->opcode() == BinaryOperator::BinaryOps::Add) &&
          ...
          if (4 * loop->blocks()[1]->insts().size() > 200) {
            continue;
          }
          if (update_num != 1)
            continue;
          ...
          DBG_ASSERT(*cmp_pos == cmp_inst,
                     "find icmp instruction position failed");
          ...
          auto tmp = BinaryOperator::Create(BinaryOperator::BinaryOps::Sub,
                                            end_var, init_var);
          ...
          auto count = BinaryOperator::Create(BinaryOperator::BinaryOps::SDiv,
                                              tmp, size);
          count->set_type(init_var->type());

          auto mult = BinaryOperator::Create(BinaryOperator::BinaryOps::Mul,
                                             count, size);
          mult->set_type(init_var->type());

          auto new_loop_end = BinaryOperator::Create(
              BinaryOperator::BinaryOps::Add, mult, init_var);
          ...
          auto cmp_rem =
              std::make_shared<ICmpInst>(front::Operator::NotEqual, rem, one);
          ...
          DBG_ASSERT(!loop_body->predecessors().empty(),
                     "loop body has no predecessor");
          check_rem_block->AddPredecessor(loop_body->predecessors().front());

          // remove header from while end
          while_end->RemovePredecessor(header);

          // renew while_end's predecessor
          while_end->AddPredecessor(check_rem_block);
          while_end->AddPredecessor(last_loop_block);
          ...
          DBG_ASSERT(_last_phi_map.size() == _phi_origin_map.size(),
          ...
          for (const auto &[k, v] : _phi_map) {
            DBG_ASSERT(_last_phi_map.find(k) != _last_phi_map.end(),
                       "found last value of phi failed");
            SSAPtr last_value = _last_phi_map[k];
            auto   phi        = dyn_cast<PhiNode>(k);
            for (const auto &use : phi->uses()) {
              // find in last body
              auto res = std::find_if(last_loop_block->inst_begin(),
                                      last_loop_block->inst_end(),
            ...
            for (auto it = phi->uses().begin(); it != phi->uses().end(); it++) {
              ...
              if (_should_not_replace.find(user) != _should_not_replace.end())
                continue;
  ...
  void CloneInstruction(const InstPtr &inst, BasicBlock *block) {
    ...
    if (auto binary_inst = dyn_cast<BinaryOperator>(inst)) {
      result = BinaryOperator::Create(binary_inst->opcode(),
                                      GetSSA(binary_inst->LHS()),
                                      GetSSA(binary_inst->RHS()));
      ...
      if (access_inst->has_multiplier())
        indexs.push_back(access_inst->multiplier());
      result = std::make_shared<AccessInst>(access_inst->acc_type(),
                                            GetSSA(access_inst->ptr()), indexs);
      ...
      result = std::make_shared<StoreInst>(GetSSA(store_inst->data()),
                                           GetSSA(store_inst->pointer()));
...
public:
  PassInfoPtr CreatePass(PassManager *) override {
    ...
    auto passinfo = std::make_shared<PassInfo>(pass, "SimpleUnrolling", false,
                                               2, LOOP_UNROLLING);
