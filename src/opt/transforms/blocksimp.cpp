...
/*
 * If Clean finds a block that ends in a branch, and both sides of the branch
 * target the same block, it replaces the branch with a jump to the target
 * block. This situation arises as the result of other simplifications. For
 * example, Bi might have had two successors, each with a jump to B j . If
 * another transformation had already emptied those blocks, then empty-block
 * removal, discussed next, might produce the initial graph shown in the margin.
 */
...
/*
 * If Clean finds a block Bi that ends in a jump to B j and B j has only one
 * predecessor, it can combine the two blocks, as shown in the margin. This
 * situation can arise in several ways. Another transformation might eliminate
 * other edges that entered B j , or Bi and B j might be the result of folding a
 * redundant branch (described previously). In either case, the two blocks can
 * be combined into a single block. This eliminates the jump at the end of Bi .
 */
  ...
  for (const auto &block : blocks) {
    ...
    if (auto jumpInst = dyn_cast<JumpInst>(BB->terminator())) {
      ...
      if (CollectPredecessors(F, target.get()).size() == 1) {
        ...
        for (const auto &block_in_func : *F) {
          if (block_in_func && block_in_func.get() == BB) {
            merged_block = block_in_func;
            break;
          }
        }
        DBG_ASSERT(merged_block != nullptr,
                   "merged block not found in function");

        for (auto *succ : target_successors) {
          if (!succ)
            continue;
          static_cast<void>(
              succ->ReplacePredecessor(target.get(), merged_block));
        }

        // set it to nullptr
        std::for_each(F->begin(), F->end(), [target](BlockPtr &block_in_func) {
          if (block_in_func == target) {
            block_in_func = nullptr;
          }
        });
...
/*
 * If Clean finds a block that contains only a jump, it can merge the block into
 * its successor. This situation arises when other passes remove all of the
 * operations from a block Bi . Consider the left graph of the pair shown in the
 * margin. Since Bi has only one successor, B j , the transformation retargets
 * the edges that enter Bi to Bj and deletes Bi from Bj’s set of predecessors.
 * This simplifies the graph. It should also speed up execution.
 * In the original graph, the paths through Bi needed two control-flow
 * operations to reach Bj. In the transformed graph, those paths use one
 * operation to reach Bj.
 */
void BlockSimplification::RemoveEmptyBlock(const FuncPtr &F) {
  ...
  for (const auto &block : blocks) {
    ...
    if (auto jumpInst = dyn_cast<JumpInst>(BB->terminator())) {
      ...
      // check phi node
      if (IsSSA<PhiNode>(target->insts().front()))
        continue;
      ...
      if (BB->insts().size() == 1) {
        ...
        // set it to nullptr
        std::for_each(F->begin(), F->end(), [BB](BlockPtr &block_in_func) {
          if (block_in_func && block_in_func.get() == BB) {
            block_in_func = nullptr;
          }
        });
...
void BlockSimplification::ReplaceSuccessor(BlockPtr   &predecessor,
                                           BlockPtr   &successor,
                                           BasicBlock *cur) {
  ...
  DBG_ASSERT(terminator->ReplaceSuccessor(cur, successor),
             "current block is not a successor");
...
void BlockSimplification::RemovePhiNode(BasicBlock               *block,
                                        std::vector<BasicBlock *> successors) {
  for (auto &succ : successors) {
    for (auto inst_it = succ->insts().begin();
         inst_it != succ->insts().end();) {
      auto phi_node = dyn_cast<PhiNode>(*inst_it);
      if (!phi_node)
        break;

      // 1. if the successor has only two predecessors, then replace the
      // phi-node with the other value.
      // 2. if the successor has multiple predecessors, then remove the
      // corresponding value.
      if (phi_node->size() == 2) {
...
std::vector<BlockPtr>
BlockSimplification::CollectPredecessors(const FuncPtr &F, BasicBlock *target) {
  std::vector<BlockPtr> preds;
  for (const auto &pred : *F) {
    if (!pred || pred.get() == target)
      continue;
