              ...
              case Instruction::Store: {
                ...
                if (store->pointer() == alloca) {
                  if (store->data()->IsArgument()) {
                    auto new_type = MakePointer(
                        MakePrimType(Type::Int32, pointeeType->IsRightValue()));
...
public:
  PassInfoPtr CreatePass(PassManager *) override {
    ...
    auto passinfo = std::make_shared<PassInfo>(pass, "DirtyArrayConvert", false,
                                               0, DIRTY_ARRAY_CONV);
    return passinfo;
  }
};
