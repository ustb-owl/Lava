...
InstPtr
ReduceSignedDivByPowerOfTwo(const std::shared_ptr<BinaryOperator> &binary_inst,
                            BasicBlock *block, InstList::iterator pos) {
  auto rhs = lava::dyn_cast<ConstantInt>(binary_inst->RHS());
  if (rhs == nullptr ||
      binary_inst->opcode() != BinaryOperator::BinaryOps::SDiv)
    return nullptr;
  if (rhs->value() <= 1 || !IsPositivePowerOfTwo(rhs->value()))
    return nullptr;
  ...
  auto sign = BinaryOperator::Create(Instruction::AShr, binary_inst->LHS(),
                                     make_const(sign_shift));
  auto bias =
      BinaryOperator::Create(Instruction::And, sign, make_const(bias_mask));
  auto adjusted =
      BinaryOperator::Create(Instruction::Add, binary_inst->LHS(), bias);
  auto reduced = BinaryOperator::Create(Instruction::AShr, adjusted,
                                        make_const(shift_amount));
  ...
  for (const auto &BB : *F) {
    for (auto it = BB->inst_begin(); it != BB->inst_end();) {
      ...
      if (auto binary_inst = dyn_cast<BinaryOperator>(*it)) {
        if (auto reduced =
                ReduceSignedDivByPowerOfTwo(binary_inst, BB.get(), it)) {
