...
void FunctionInlining::CopyInstruction(const InstPtr &inst) {
  ...
  case ClassId::BranchInstId: {
    ...
    auto true_block  = blk_map[orig_branch->true_block()];
    auto false_block = blk_map[orig_branch->false_block()];

    // update predecessor
    true_block->AddPredecessor(insert_point);
    false_block->AddPredecessor(insert_point);
  ...
  case ClassId::JumpInstId: {
    ...
    copied_inst    = module->CreateJump(blk_map[orig_jump->target()]);
    break;
  }
  case ClassId::BinaryOperatorId: {
    ...
    copied_inst   = BinaryOperator::Create(
        BinaryOperator::BinaryOps(orig_bin->opcode()), lhs, rhs);
  ...
  case ClassId::CastInstId: {
    ...
    copied_inst    = module->AddInst<CastInst>(
        CastInst::CastOps(orig_cast->opcode()), value);
    copied_inst->set_type(inst->type());
    break;
  }
  ...
  case ClassId::PHINodeId: {
    ...
    dom_frontier->InsertInst(dom_frontier->inst_begin(), phi);
...
void FunctionInlining::CopyFunction(const FuncPtr &F) {
  ...
  DBG_ASSERT(blk_map.size() == blocks.size(),
             "This number of basic blocks is wrong");
...
void FunctionInlining::ConnectUD(const CallInstPtr &call_inst) {
  ...
  auto callee_entry = callee->entry();
  auto callee_shared =
      std::find_if(F->begin(), F->end(), [BB](const BlockPtr &block) {
        return block && block.get() == BB;
      });

  callee_entry->SetBlockName(inline_entry_name +
                             std::to_string(inline_entry_id));
  ...
  auto exit_shared =
      std::find_if(blocks.begin(), blocks.end(),
                   [exit](const BlockPtr &bb) { return bb.get() == exit; });
  ...
  for (auto &it : orig_succ) {
    it->ReplacePredecessor(BB, *exit_shared);
  }
...
public:
  PassInfoPtr CreatePass(PassManager *) override {
    ...
    auto passinfo = std::make_shared<PassInfo>(pass, "FunctionInlining", false,
                                               99, FUNCTION_INLINING);
...
public:
  bool runOnModule(Module &M) final {
    ...
    auto  res       = functions.erase(
        std::remove_if(functions.begin(), functions.end(),
                       [](const FuncPtr &F) { return F->is_copied(); }),
        functions.end());
...
public:
  PassInfoPtr CreatePass(PassManager *) override {
    ...
    auto passinfo = std::make_shared<PassInfo>(pass, "FunctionCleanUp", false,
                                               99, FUNCTION_CLEANUP);
    passinfo->Requires("FunctionInfoPass");
    return passinfo;
  }
