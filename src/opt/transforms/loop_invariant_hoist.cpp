...
void CollectLoopsPostOrder(const lava::opt::LoopPtr &loop, std::vector<lava::opt::Loop *> &loops) {
...
bool LoopInvariantHoist::DominatesPreheader(const InstPtr &inst, BasicBlock *preheader) const {
  auto it = _dom_info.at(_cur_func).domBy.find(preheader);
  if (it == _dom_info.at(_cur_func).domBy.end()) return false;
...
BasicBlock *LoopInvariantHoist::GetLoopPreheader(const Loop *loop) const {
  ...
  for (const auto &pred : *header) {
    auto block = dyn_cast<BasicBlock>(pred.value()).get();
    if (IsInLoop(block, loop)) continue;
    if (preheader != nullptr) return nullptr;
    preheader = block;
  }
  return preheader;
}
...
bool LoopInvariantHoist::HoistInstructionIfInvariant(
  ...
  if (!IsInLoop(inst->getParent(), loop)) return DominatesPreheader(inst, preheader);
  if (hoisted.contains(raw_inst)) return true;
  if (!IsHoistableInstruction(inst) || IsSSA<PhiNode>(inst) || inst->isTerminator()) return false;
  if (!active.insert(raw_inst).second) return false;

  for (const auto &operand : *inst) {
    auto op_inst = dyn_cast<Instruction>(operand.value());
    if (!op_inst) continue;
    if (IsInLoop(op_inst->getParent(), loop)) {
      if (!HoistInstructionIfInvariant(op_inst, loop, preheader, hoisted, active)) {
...
bool LoopInvariantHoist::runOnFunction(const FuncPtr &F) {
  ...
  for (auto *loop : loops) {
    ...
    if (preheader == nullptr || preheader->terminator() == nullptr) continue;

    std::vector<InstPtr> candidates;
    for (auto *block : loop->blocks()) {
      for (const auto &inst : block->insts()) {
        auto instruction = dyn_cast<Instruction>(inst);
        if (instruction->isTerminator() || IsSSA<PhiNode>(instruction)) continue;
