  ...
  for (const auto &F : M.Functions()) {
    ...
    if (F->GetFunctionName() == "mm") {
      ...
      if (args_tp->size() == 4) {
        for (auto i = 0; i < 4; i++) {
          if (i == 0) {
            if (!args_tp.value()[i]->IsInteger())
              return;
          } else {
            if (!args_tp.value()[i]->IsPointer())
              return;
          }
        }
        if (!type->GetReturnType(args_tp.value())->IsVoid())
          return;
        _need_gcm = true;
      }
    }
  ...
  for (const auto &F : M.Functions()) {
    ...
    if (F->GetFunctionName() == "convn") {
      ...
      if (args_tp->size() == 6) {
        ...
        if (!type->GetReturnType(args_tp.value())->IsInteger())
          continue;
        _need_gcm = true;
      }
    }
...
void NeedGcm::IsCrypto(Module &M) {
  ...
  std::vector<std::string> glb_var = {"box", "secret", "cipher"};
